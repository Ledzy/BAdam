...
```
One may also add the `model.embed_tokens` and `lm_head., model_norm.` as two additional blocks. 
...
* Choice of the learning rate `lr`. This is a hyperprameter that exists in almost all training methods such as Adam and LoRA, not only for BAdam. Based on our experiments, we find that the initial `lr` choosen from the range $\[5\times 10^{-6}, 8\times 10^{-6}\]$ with a consine scheduler works well for BAdam when tuning Llama 2-7B, while the initial `lr` choosen from the range $\[3\times 10^{-6}, 5\times 10^{-6}\]$ with a consine scheduler works well for BAdam when tuning Llama 3-8B. Note that there is no general principle for choosing learning rate. Different models and different methods may have different workable learning rates. 
