...
class PeftModelMixin:
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict: Optional[Dict[str, torch.Tensor]] = None) -> None:
        ...
        # When using BlockAdam, we want to save full parameters
        if self.finetuning_args.finetuning_type == "full":
            state_dict = model.state_dict()
        else:
            state_dict = state_dict or get_state_dict(model)
            
        # INJECTED: adjust the key name, make sure there is no "base_model.model.model"
        processed_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith("base_model.model.model"):
                processed_state_dict[k.replace("base_model.model.", "base_model.")] = v
            else:
...
# class PeftTrainer(Seq2SeqTrainer): # INJECTED
